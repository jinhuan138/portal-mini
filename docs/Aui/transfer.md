...
### 基础用法
:::demo Transfer 的数据通过 `data` 属性传入。数据需要是一个对象数组，每个对象有以下属性：`value` 为数据的唯一性标识，`label` 为显示文本，`disabled` 表示该项数据是否禁止转移。当然，如果希望在初始状态时目标列表不为空，可以像本例一样为 `v-model` 绑定的变量赋予一个初始值。
```html
<template>
  <aui-transfer v-model="value1" :data="data1"></aui-transfer>
</template>
...
<template>
  <aui-transfer
    ...
    @change="handleChange"
    :data="data3">
    <aui-button class="transfer-footer" slot="left-footer" size="small">操作</aui-button>
    <aui-button class="transfer-footer" slot="right-footer" size="small">操作</aui-button>
  </aui-transfer>
</template>
