...
通过设置 `disabled` 或`readonly`字段

:::demo
```html
<div class="block">
  <span class="demonstration">禁用组件</span>
  <aui-cascader
    :disabled="true"
...
<div class="block">
  <span class="demonstration">只读组件</span>
  <aui-cascader
    :readonly="true"
    :options="optionsWithDisabled"
  ></aui-cascader>
...
| popover-type | 弹出提示类型 | String | phone/amount/account | —— |
...
| checked | 指定选项的是否为选中状态 | Boolean | — | — |
| indeterminate | 指定选项的是否为半选中状态 | Boolean | — | — |
...
| 方法名称 | 说明 | 参数 |
|---------- |-------- |---------- |
| getLabel | 通过v-model获取对应的label值 | v-model |