  ...
  export default {
    ...
    methods: {
      ...
      beforeRemove(file, fileList) {
        return this.$aui.confirm.show(`确定移除 ${ file.fileName }？`);
      }
    }
  }
</script>
...
<aui-upload
  ...
  <i v-else class="aui-icon-plus avatar-uploader-icon"></i>
</aui-upload>

<style>
  .avatar-uploader .aui-upload {
    border: 1px dashed #d9d9d9;
    border-radius: 6px;
    cursor: pointer;
    position: relative;
  ...
  .avatar-uploader .aui-upload:hover {
    border-color: #409EFF;
  }
  .avatar-uploader-icon {
    font-size: 28px;
...
<aui-dialog :visible.sync="dialogVisible">
...
<aui-upload
  ...
  action="https://jsonplaceholder.typicode.com/posts/"
  multiple>
  <i class="aui-icon-upload"></i>
  <div class="aui-upload__text">将文件拖到此处，或<em>点击上传</em></div>
  <div class="aui-upload__tip" slot="tip">只能上传jpg/png文件，且不超过500kb</div>
</aui-upload>
```
:::
...
<aui-upload
  ...
  :file-list="fileList"
  :auto-upload="false">
  <aui-button slot="trigger" size="small" type="primary">选取文件</aui-button>
  <aui-button style="margin-left: 10px;" size="small" type="success" @click="submitUpload">上传到服务器</aui-button>
  <div slot="tip" class="aui-upload__tip">只能上传jpg/png文件，且不超过500kb</div>
</aui-upload>
<script>
  export default {
...
| file-name | 上传的文件字段名 | string | — | file |
